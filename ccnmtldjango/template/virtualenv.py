...
def _install_req(py_executable, unzip=False, distribute=False, 
                 setuptools_egg_path=None, setuptools_version=None):
    if not distribute:
        ...
        if setuptools_version is not None:
            bootstrap_script = bootstrap_script.replace(
                ...
                'DEFAULT_VERSION = "%s"' % setuptools_version)
    ...
    if setup_fn is not None:
        if setuptools_egg_path is None:
            setup_fn = _find_file(setup_fn, search_dirs)
        else:
            setup_fn = setuptools_egg_path
...
def install_setuptools(py_executable, unzip=False, egg_path=None, version=None):
    _install_req(py_executable, unzip, setuptools_egg_path=egg_path, setuptools_version=version)
...
def main():
    ...
    parser.add_option(
        '--setuptools-egg-path',
        dest="setuptools_egg_path",
        help="Path to a setuptools egg that should be installed instead of "
        "the default (only useful if you are not using distribute)")

    parser.add_option(
        '--setuptools-version',
        dest="setuptools_version",
        help="The setuptools version string that should be installed instead of "
        "the default (only useful if you are not using distribute)")
    ...
    create_environment(home_dir, site_packages=not options.no_site_packages, clear=options.clear,
                       ...
                       setuptools_egg_path=options.setuptools_egg_path,
...
def create_environment(home_dir, site_packages=True, clear=False,
    ...
    if use_distribute or os.environ.get('VIRTUALENV_USE_DISTRIBUTE'):
        ...
        install_setuptools(py_executable, unzip=unzip_setuptools, egg_path=setuptools_egg_path, version=setuptools_version)
